...
const getStatusCardTheme = (status = "") => {
  switch (status?.toLowerCase()) {
    ...
    default:
      return {
        card: "border-[#cdd7f2] bg-[#f2f4fb]",
        icon: "bg-[#4565BF]/[0.08] text-[#4565BF]",
        dot: "bg-[#4565BF]",
      };
  }
};
...
const DetailField = ({ label, value, capitalize = false }) => (
  <div className="rounded-[16px] border border-[#4565BF]/[0.08] bg-[#f7f8fc] px-4 py-4">
    <p className="inter-medium-font text-[10px] uppercase tracking-[0.11em] text-slate-400">
      {label}
    </p>
    <p
      className={`inter-medium-font mt-2 text-[13px] leading-5 text-slate-900 ${capitalize ? "capitalize" : ""}`}
    >
      {value || "N/A"}
    </p>
  </div>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            {[0, 1].map((i) => (
              <div
                key={i}
                className="flex items-center gap-3.5 rounded-2xl border border-[#e0e6f7] bg-white px-4 py-3.5 sm:min-w-[200px]"
              >
      ...
      <div className="overflow-hidden rounded-[22px] border border-[#4565BF]/10 bg-white">
        ...
        <div className="hidden md:block">
          ...
          {[0, 1, 2].map((i) => (
            <div
              key={i}
              className="flex items-center border-b border-[#4565BF]/[0.06] px-5 py-5 gap-4"
            >
              <div
                className={`h-4 flex-1 ${p}`}
  ...
  <main className="inter-reg-font min-w-0 flex-1 bg-[#E8EDFA]">
    <div className="mx-auto w-full max-w-[1560px] p-4 sm:p-5 lg:p-6">
      <div className="flex min-h-[380px] flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white px-5 text-center">
        ...
        <h1 className="inter-bold-font mt-5 text-[22px] text-slate-900">
          Order not available
        </h1>
                ...
                right={
                  <div className="grid w-full grid-cols-1 gap-3 sm:flex sm:w-auto sm:items-center">
                    ...
                    {(() => {
                      ...
                      return (
                        <div
                          className={`flex min-w-0 w-full items-center gap-3.5 rounded-2xl border px-4 py-3.5 sm:w-auto sm:min-w-[200px] sm:px-5 sm:py-4 ${theme.card}`}
                        >
                          <span
                          ...
                          <div className="flex flex-col gap-1.5">
                            <span className="inter-medium-font whitespace-nowrap text-[10px] uppercase tracking-[0.12em] text-slate-400">
                              Order Status
                            </span>
                            <div className="flex items-center gap-1.5">
                              ...
                              />
                              <span
                                className={`inter-semibold-font whitespace-nowrap text-[12px] capitalize ${getStatusColor(
                                  currentOrder?.status || "",
                                )
                                  .split(" ")
                    ...
                    {/* Payment Status card */}
                    {(() => {
                      const theme = getStatusCardTheme(
                        currentOrder?.payments?.status,
                      );
                      return (
                        <div
                          className={`flex min-w-0 w-full items-center gap-3.5 rounded-2xl border px-4 py-3.5 sm:w-auto sm:min-w-[210px] sm:px-5 sm:py-4 ${theme.card}`}
                        >
                          <span
                            className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${theme.icon}`}
                          >
                ...
                {activeTab === 0 && (
                  <div className="overflow-hidden rounded-[22px] border border-[#4565BF]/10 bg-white">
                    <div className="flex items-center gap-3 border-b border-[#4565BF]/[0.07] bg-[#f7f8fc] px-5 py-4">
                      ...
                      <h2 className="inter-bold-font text-[18px] leading-6 text-slate-950">
                        Order details
                      </h2>
                    </div>
                      ...
                      <table className="w-full min-w-[700px] border-collapse text-left">
                        <thead className="bg-white">
                          <tr className="border-b border-[#4565BF]/[0.07]">
                            <th className="inter-medium-font px-5 py-4 text-[12px] lg:text-[13px] uppercase tracking-[0.11em] text-slate-400">
                              Item
                            </th>
                            <th className="inter-medium-font w-[150px] px-5 py-4 text-center text-[12px] lg:text-[13px] uppercase tracking-[0.11em] text-slate-400">
                              Quantity
                            </th>
                            <th className="inter-medium-font w-[180px] px-5 py-4 text-right text-[12px] lg:text-[13px] uppercase tracking-[0.11em] text-slate-400">
                              Amount
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                          ...
                          <tr className="border-b border-[#4565BF]/[0.07] bg-[#f7f8fc]/60">
                            <td className="px-5 py-4">
                              <span className="inter-medium-font text-[13px] lg:text-[14px] text-slate-600">
                                Shipping fee
                              </span>
                            </td>
                            <td />
                          ...
                          <tr className="bg-[#4565BF]/[0.035]">
                            <td className="px-5 py-5">
                              <span className="inter-bold-font text-[16px] lg:text-[17px] text-slate-950">
                                Order total
                              </span>
                            </td>
                            <td />
                          ...
                          >
                            ...
                            <div className="mt-4 flex items-end justify-between gap-3">
                              <div>
                                ...
                                <p className="inter-medium-font mt-1.5 text-[12px] text-slate-700">
                                  {product?.quantity}
                                </p>
                              </div>
                      ...
                      <div className="border-t border-[#4565BF]/[0.07] bg-[#f7f8fc] p-4">
                        <div className="space-y-3.5">
                          ...
                          <div className="flex items-end justify-between gap-4">
                            ...
                            <span className="inter-bold-font text-[20px] text-[#4565BF]">
                              £{formatCurrency(total)}
                            </span>
                          </div>
                        </div>
